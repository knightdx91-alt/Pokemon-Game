...
- **Startup override** (`src/main.js init`): `index.html?map=<Name>&region=<region>`
  loads any map directly instead of Pallet Town — use it to test a built map,
  e.g. `index.html?map=VerdantHollow&region=custom`.
