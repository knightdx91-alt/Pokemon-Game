...
(function () {
    ...
    async function transitionToWarp(warp) {
        ...
        try {
            ...
            // Snap prev to current so no lerp artifact after warp
    ...
    async function transitionToConnection(connInfo) {
        ...
        try {
            ...
            // Snap prev to current so no lerp artifact after transition
            player.prevX = player.x;
            player.prevY = player.y;
            player.moveStartTime = 0;
    ...
    function gameLoop(timestamp) {
            ...
            if (elapsed >= MOVE_COOLDOWN_MS) {
                ...
                if (dx !== 0 || dy !== 0) {
                    ...
                    if (oob) {
                        ...
                        // Record previous position for smooth interpolation
                        player.prevX = player.x;
                        player.prevY = player.y;
                        player.x = nx;
                        player.y = ny;
                        ...
                        // Advance walk animation: 0→1→2→1→0...
        ...
        // Update logical camera (used for warp/connection checks)
        GameCamera.update(player.x, player.y, GameMap.width, GameMap.height);
